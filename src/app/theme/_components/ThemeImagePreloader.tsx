...
/**
 * 모든 프레임·배경 조합의 이미지를 숨겨서 미리 렌더링해 테마 전환 시 이미지 깜빡임(FOUI)을 방지하는 컴포넌트
 */
    ...
    <div className="invisible absolute" aria-hidden>
      {FRAME_KEYS.flatMap((frameKey, frameIndex) =>
        BG_KEYS.map((bgKey, bgIndex) => {
          ...
          const backgroundUrl = `/images/previews/backgrounds/${frameKey}-${bgKey}.png`;
          const overlayUrl = `/images/previews/backgrounds/${frameKey}-${bgKey}.png`;
